...
def load_snli(path, sample_size=None):
    import json
    mapping = {"entailment": 1.0, "neutral": 0.0, "contradiction": 0.0}
...
# ============================================================
#                   TRUE MIZAN SIMILARITY
# ============================================================

def mizan_similarity(e1, e2, p=2, eps=1e-6):
    """This EXACT formula is used in your Medium Articles."""
...
# ============================================================
#               OFFICIAL MIZAN CONTRASTIVE LOSS
# ============================================================

class MizanContrastiveLoss(nn.Module):
    def __init__(self, margin=0.5, p=2):
        super().__init__()
        self.margin = margin
        self.p = p

    def forward(self, e1, e2, labels):
        sim = mizan_similarity(e1, e2, p=self.p)

        pos_loss = 1 - sim
        neg_loss = torch.clamp(self.margin - sim, min=0)

        return torch.where(labels == 1, pos_loss, neg_loss).mean()
